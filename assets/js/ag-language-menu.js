(() => {
  ...
  const VERSION = '1.0.0';
  const SELECTOR = 'select[data-ag-language-menu], select#languageSelect';
  const LANGUAGES = [
  ...
  function installStyles() {
    if (document.getElementById('ag-language-menu-master-styles')) return;
    const style = document.createElement('style');
    style.id = 'ag-language-menu-master-styles';
    style.textContent = [
      ...
      '.ag-language-button{min-width:166px;min-height:42px;display:flex;align-items:center;gap:8px;border:1px solid var(--ag-lang-border,var(--border,var(--line,#dbe3ef)));border-radius:12px;padding:7px 34px 7px 10px;background:var(--ag-lang-bg,var(--surface,#fff));color:var(--ag-lang-text,var(--text,#18233b));font:inherit;font-weight:650;cursor:pointer;text-align:left;position:relative;white-space:nowrap;box-shadow:0 4px 14px rgba(25,43,74,.06)}',
      '.ag-language-button:hover{border-color:var(--ag-lang-accent,var(--primary,#2667ff));background:var(--ag-lang-hover,var(--surface-2,#f7f9fd))}',
      '.ag-language-button:focus-visible{outline:3px solid color-mix(in srgb,var(--ag-lang-accent,var(--primary,#2667ff)) 28%,transparent);outline-offset:2px}',
      ...
      '.ag-language-options{position:absolute;z-index:10000;top:calc(100% + 6px);min-width:190px;width:max-content;max-width:min(240px,calc(100vw - 20px));padding:5px;border:1px solid var(--ag-lang-border,var(--border,var(--line,#dbe3ef)));border-radius:11px;background:var(--ag-lang-bg,var(--surface,#fff));box-shadow:0 14px 34px rgba(12,24,56,.22);display:none}',
      ...
      '.ag-language-option:hover,.ag-language-option:focus-visible{outline:none;background:var(--ag-lang-hover,var(--surface-2,#f7f9fd))}',
      '.ag-language-option:focus-visible{box-shadow:inset 0 0 0 2px color-mix(in srgb,var(--ag-lang-accent,var(--primary,#2667ff)) 38%,transparent)}',
      '.ag-language-option.selected{background:var(--ag-lang-selected,color-mix(in srgb,var(--ag-lang-accent,var(--primary,#2667ff)) 12%,var(--ag-lang-bg,var(--surface,#fff))));color:var(--ag-lang-accent,var(--primary,#2667ff));font-weight:750}',
  ...
  function positionMenu(menu) {
    ...
    const roomRight = viewportWidth - rect.left;
    menu.classList.toggle('align-left', roomRight >= estimatedWidth);
    menu.classList.toggle('align-right', roomRight < estimatedWidth);
  }
  ...
  function enhanceSelect(select) {
    ...
    const languages = availableLanguages(select);
    const menu = document.createElement('div');
    menu.className = 'ag-language-menu align-right';
    menu.dataset.agLanguageMenuVersion = VERSION;
    ...
    const label = select.getAttribute('aria-label') || 'Language';
    button.setAttribute('aria-label', label);

    const options = document.createElement('div');
    options.className = 'ag-language-options';
    options.id = (select.id || 'agLanguage') + 'MenuOptions';
    ...
    languages.forEach(item => {
      ...
      optionButton.innerHTML = optionMarkup(item);
    ...
    function updateDisplay() {
      let current = languageByValue(select.value);
      if (!languages.some(item => item.value === current.value)) current = languages[0];
      button.innerHTML = optionMarkup(current) + '<span class="ag-language-chevron" aria-hidden="true">▼</span>';
      items().forEach(optionButton => {
        const selected = optionButton.dataset.language === current.value;
  ...
  function bindObserver() {
    ...
    const observer = new MutationObserver(mutations => {
      ...
      if (shouldScan) enhanceAll(document);
    });
  ...
  window.AppsGamesLanguageMenu = Object.freeze({
    version: VERSION,
